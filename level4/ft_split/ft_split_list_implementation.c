...
/*   Updated: 2019/03/15 17:35:48 by maparmar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_rev_wstr(char *str)
{
	...
	{
		j = 0;
		while (ft_space(str[i]) && str[i]) // till when there is a white space
			i++;
		start = i;
		word_len = 0;
...
void print_rev(t_list *head) // print via recursion
{
	...
	if (head)
	{
		...
		print_str(head->word); // print the word after all recursive call because the words are store in a reverse manner in the list
	}
}
